    ...
    public View getView(final int position, View convertView, ViewGroup parent)
    {
        ...
        connBtn.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                {
                    wiFiUtil.addWiFiNetwork(item.SSID,"lyl1886135", WiFiUtil.Data.WIFI_CIPHER_WPA2);
                }
                else if (item.capabilities.contains("WPA"))
                {
                    wiFiUtil.addWiFiNetwork(item.SSID,"lyl1886135", WiFiUtil.Data.WIFI_CIPHER_WPA);
                }
                else if (item.capabilities.contains("WEP"))
                {
                    /* WIFICIPHER_WEP 加密 */
                    wiFiUtil.addWiFiNetwork(item.SSID,"lyl1886135", WiFiUtil.Data.WIFI_CIPHER_WEP);
                }
                else
                {
